...
class NukeWishesTest extends MaintenanceBaseTestCase {
	...
	public function testExecute(): void {
		...
		$this->insertPage(
			...
			"<wish title=\"My first wish\" proposer=\"{$proposer->getName()}\" created='2025-06-25T12:59:59Z' />"
		);
		$this->insertPage(
			Title::newFromText( 'Community Wishlist/Wishes/W2' ),
			"<wish title=\"My second wish\" proposer=\"{$proposer->getName()}\" created='2025-06-25T12:59:59Z' />"
		);
		$this->newSelectQueryBuilder()
			->select( 'COUNT(*)' )
			->from( 'communityrequests_wishes' )
